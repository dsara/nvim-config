return {
  {
    ...
    config = function()
      ...
      vim.keymap.set(
        'n',
        '<leader>sf',
        function() require('resession').load(nil, { dir = 'dirsession', reset = false }) end,
        { noremap = true, silent = true, desc = 'Load folder session (ressession)' }
      )
  ...
  },
  {
    ...
    config = function()
      vim.keymap.set(
        ...
        { noremap = true, silent = true, desc = 'Load session for directory (persistence)' }
      )
    end,
    enabled = false,
  },
